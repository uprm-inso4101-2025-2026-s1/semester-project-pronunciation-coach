...
==== 2.2.8 Requirements Conflict Analysis and Resolution

### Overview
During requirements evaluation, three significant conflicts were identified 
between gamification system requirements. These conflicts arose from competing 
stakeholder needs (accountability vs. flexibility) and system design trade-offs 
(quality vs. participation). This section documents the systematic resolution 
process following Van Lamsweerde's conflict management methodology (Chapter 3).

### Interaction Matrix

|===
| | Streak | XP | Duration | Accuracy | *Total*

| *Daily Streak System* | - | 1 | 0 | 1 | *2*
| *XP Point Award* | 1 | - | 1 | 0 | *2*
| *Practice Duration* | 0 | 1 | - | 0 | 1
| *Accuracy Feedback* | 1 | 0 | 0 | - | 1
|===
**Key Finding:** Streak and XP systems exhibit the most requirement overlaps, 
indicating these as high-priority areas for conflict resolution.

---

### Conflict #1: Streak Reset Policy

**Type:** Weak Conflict (Divergence)

**Conflicting Statements:**
- **Statement A:** "System must reset streak to 0 when user misses a day" 
  (Source: David persona - Section 2.2.2, dedicated learner viewpoint)
- **Statement B:** "System should allow grace period for missed practice days"
  (Source: Maria persona - Section 2.2.2, commuter student viewpoint)

**Boundary Condition:** User with established streak (30+ days) misses practice 
due to uncontrollable life event (illness, family emergency, work crisis).

**Root Cause:** Conflicting stakeholder objectives - accountability-focused 
learners value strict discipline, while schedule-constrained learners require 
flexibility for unpredictable life circumstances.

**Proposed Resolutions:**

1. **Restore Tactic:** Streak marked as "broken" but recoverable through 
   double practice completion on following day
   - Pros: Acknowledges miss while rewarding recovery effort, maintains 
     motivation through "comeback" mechanic
   - Cons: Complex streak state management (Active/Broken/Recovered states)

2. **Weaken Tactic:** Allow one "streak freeze" token per month
   - Pros: Simple rule, predictable for users
   - Cons: Can be gamed by saving tokens, doesn't scale with streak length

3. **Specialize Tactic:** Grace period only available for Casual pace users
   - Pros: Aligns with learning pace selection philosophy
   - Cons: May feel unfair to dedicated learners, encourages gaming pace selection

**Selected Resolution:** Option 1 (Restore)

**Rationale:** 
- Best contribution to user retention NFR (Section 2.2.5)
- Supports growth mindset learning philosophy
- Adds engaging "recovery challenge" mechanic
- Resolves conflict without dropping either stakeholder need

**Updated Requirements (replaces conflicting statements):**

> "The system shall maintain streak states: Active, Broken, and Recovered. 
> When a user misses daily practice, their streak transitions to Broken state 
> with current count preserved. Users may recover a Broken streak by completing 
> two challenges on the following day, transitioning to Recovered state with 
> count continuing. Broken streaks not recovered within 24 hours reset to 0."

**Implementation Notes:** 
- Add StreakState enum: {Active, Broken, Recovered}
- Update UserProgress class (Section 2.3.2) with state machine logic
- UI must clearly communicate streak status and recovery opportunity

---

### Conflict #2: XP Point Distribution

**Type:** Weak Conflict (Divergence)

**Conflicting Statements:**
- **Statement A:** "XP awarded based on pronunciation accuracy percentage"
  (Source: Section 2.2.6 - quality-focused assessment requirement)
- **Statement B:** "XP awarded for completing any daily challenge"
  (Source: Section 1.2.3 - engagement-focused gamification strategy)

**Boundary Condition:** User with low pronunciation accuracy (40%) completes 
daily challenge through full participation.

**Root Cause:** Trade-off between quality metrics (encouraging improvement) 
and participation incentives (maintaining engagement).

**Selected Resolution:** Restore Tactic - Dual XP system

> "The system shall award Base XP (50 points) upon challenge completion and 
> Accuracy Bonus XP (0-50 points) calculated as: accuracy_percentage / 2. 
> Total XP per challenge ranges from 50 (minimum participation) to 100 
> (perfect accuracy)."

**Rationale:** Satisfies both quality and participation objectives, rewards 
improvement without punishing struggling learners.

---

### Conflict #3: Practice Duration Requirements

[Similar detailed documentation structure as above...]

**Selected Resolution:** Weaken Tactic - Flexible duration OR high accuracy

> "The system shall consider practice complete when EITHER minimum duration 
> requirement met for selected pace OR accuracy score exceeds 80%, allowing 
> efficient high-performing learners to complete challenges quickly."
...
**New Requirements Added:** 3  
**Requirements Modified:** 2  
**Requirements Dropped:** 0

**Most Conflicted Requirement:** Daily Streak System (2 conflicts)  
**Resolution Tactics Applied:** Restore (2x), Weaken (1x)

**Architectural Impact:**
- UserProgress class requires state machine implementation
- XP calculation logic needs dual-component tracking
- Challenge completion validation requires compound boolean logic

**Testing Implications:**
- Add integration tests for streak state transitions
- Validate XP calculation edge cases (0% accuracy, 100% accuracy)
- Test duration/accuracy OR logic with boundary values

---

==== 2.2.9 Maintenance Requirements

===== 2.2.9.1 Adaptive Maintenance

**Purpose**: Define requirements for system adaptation to evolving user needs, platform changes, and external service integration.

**AM-001: Language Pack Extensibility**
The system shall support addition of new language pairs (beyond Spanish-English) without modifying core pronunciation assessment logic. New language packs shall be loadable as plugin modules containing:
- Phonetic transcription mappings (IPA)
- Native speaker audio samples
- Language-specific pronunciation rules

**Rationale**: Future expansion to support French, Mandarin, or other languages requires architectural flexibility without core system rewrites.

**AM-002: Operating System Version Compatibility**
The system shall maintain compatibility with the two most recent major versions of iOS and Android operating systems. When a new OS version releases, the system shall be updated within 60 days to ensure continued functionality.

**Rationale**: Mobile OS fragmentation requires proactive compatibility maintenance to prevent user lockout.

**AM-003: Third-Party API Migration**
The system shall encapsulate all dependencies on external pronunciation audio sources (e.g., YouGlish) behind abstract interfaces. When external services deprecate or change APIs, only adapter layer implementations shall require modification.

**Rationale**: External service providers may change pricing, shut down, or alter APIs unpredictably. Isolation prevents cascading maintenance costs.

**AM-004: Accessibility Standards Compliance**
The system shall be updated to maintain compliance with WCAG 2.2 (or latest version) accessibility guidelines as standards evolve. Updates shall be implemented within 90 days of new guideline publication.

**Rationale**: Legal requirements (ADA compliance) and ethical obligations to support users with disabilities require adaptive maintenance.

---

===== 2.2.9.2 Corrective Maintenance

**Purpose**: Define requirements for defect identification, reporting, and resolution processes.

**CM-001: Crash Report Collection**
The system shall automatically collect anonymized crash logs including:
- Device type and OS version
- Application version
- Stack trace of failure point
- User actions preceding crash (last 5 interactions)

Crash reports shall be transmitted on next successful network connection with user consent.

**Rationale**: Rapid defect identification requires detailed diagnostic data from production environments.

**CM-002: Bug Fix Priority Classification**
The system maintenance process shall classify reported defects using severity levels:
- **Critical**: Data loss, security vulnerability, or complete feature unavailability → 24-hour response
- **High**: Major feature degradation affecting >10% users → 7-day response
- **Medium**: Minor feature issues or cosmetic defects → 30-day response
- **Low**: Enhancement requests or edge case issues → Best effort

**Rationale**: Limited maintenance resources require prioritization based on user impact.

**CM-003: Hotfix Deployment Capability**
The system architecture shall support hotfix deployment for critical defects without requiring full app store review cycles. Configuration-based fixes and server-side logic updates shall be deployable within 4 hours of defect confirmation.

**Rationale**: App store review processes (2-7 days) are too slow for critical security or data loss issues.

**CM-004: Pronunciation Algorithm Correction**
The system shall maintain a defect tracking database specifically for pronunciation assessment false positives/negatives. When accuracy issues are reported for specific words:
1. Audio samples and MFCC analysis data shall be logged
2. Linguistic experts shall review within 14 days
3. Algorithm parameter adjustments shall be deployed via server-side configuration

**Rationale**: Pronunciation assessment is probabilistic; continuous refinement prevents user frustration.

---

===== 2.2.9.3 Perfective Maintenance

**Purpose**: Define requirements for system optimization and quality improvement based on usage analytics.

**PM-001: Performance Optimization Cycle**
The system shall undergo quarterly performance audits analyzing:
- Audio processing latency (target: <2 seconds)
- UI responsiveness (target: <100ms touch response)
- Battery consumption (target: <5% per 15-minute session)
- Storage efficiency (target: <50MB app footprint)

Optimizations addressing >10% degradation from targets shall be prioritized for next release.

**Rationale**: Performance degrades over time due to feature additions; proactive optimization maintains user experience.

**PM-002: UI/UX Refinement Based on Analytics**
The system shall log user interaction patterns including:
- Feature usage frequency
- Navigation path analysis
- Task completion rates
- Error rate per UI component

Bi-annual UI reviews shall identify components with <60% task completion rates or >15% error rates for redesign.

**Rationale**: Data-driven UX improvements reduce user frustration and increase long-term engagement.

**PM-003: Gamification Effectiveness Tuning**
The system shall track gamification metric effectiveness:
- Streak retention rates (target: 60% users maintain 7-day streaks)
- XP point value perception (user satisfaction surveys)
- Badge unlock distribution (target: 80% users unlock ≥3 badges monthly)

Gamification parameters (XP values, badge thresholds) shall be adjusted quarterly based on retention analytics.

**Rationale**: Gamification effectiveness varies by user population; continuous tuning maximizes motivation impact.

**PM-004: Pronunciation Model Quality Improvement**
The system shall maintain a pronunciation model accuracy tracking database. When assessment accuracy drops below 85% for specific phoneme categories:
1. Additional native speaker audio samples shall be collected
2. MFCC feature extraction parameters shall be re-tuned
3. Machine learning models (if implemented) shall be retrained with expanded datasets

**Rationale**: Pronunciation assessment quality directly impacts user trust; continuous improvement maintains credibility.

---

===== 2.2.9.4 Preventive Maintenance

**Purpose**: Define requirements for proactive system health monitoring and failure prevention.

**PV-001: Automated Dependency Security Scanning**
The system build pipeline shall automatically scan all third-party dependencies (Flutter packages, native libraries) for known security vulnerabilities. Builds with HIGH or CRITICAL severity vulnerabilities shall fail until dependencies are updated.

**Rationale**: Reactive security patching is costly and risky; proactive scanning prevents exploitation.

**PV-002: Database Schema Migration Testing**
The system shall maintain automated test suites for SharedPreferences schema migrations covering:
- Version N → N+1 upgrades
- Multi-version skip upgrades (N → N+3)
- Corrupted data recovery scenarios

All migration paths shall be tested before production deployment.

**Rationale**: Data migration failures cause user data loss; preventive testing ensures upgrade safety.

**PV-003: API Deprecation Monitoring**
The system shall monitor deprecation notices from all external dependencies:
- Flutter framework APIs
- Android/iOS platform APIs
- Third-party service APIs (YouGlish, TTS engines)

Deprecation notices with <6-month sunset timelines shall trigger immediate refactoring tasks.

**Rationale**: Reactive API migration under deadline pressure increases defect risk; proactive monitoring enables planned transitions.

**PV-004: Capacity Planning for User Growth**
The system architecture shall be reviewed quarterly for scalability constraints. Growth projections (based on 30-day user acquisition trends) shall trigger infrastructure upgrades when:
- Projected concurrent users exceed 75% of current capacity
- Projected storage requirements exceed 80% of available quota
- Network bandwidth utilization exceeds 70% of available capacity

**Rationale**: Reactive scaling during traffic spikes causes service degradation; preventive capacity planning maintains performance.

**PV-005: Code Quality Metrics Tracking**
The system codebase shall maintain minimum quality thresholds enforced via CI/CD:
- Test coverage: ≥70% line coverage
- Cyclomatic complexity: ≤15 per function
- Code duplication: ≤5% duplicate code blocks
- Linting violations: 0 errors (warnings allowed)

Quarterly code health reviews shall address declining trends before technical debt accumulates.

**Rationale**: Code quality degradation increases future maintenance costs; preventive quality enforcement reduces long-term burden.

---

===== 2.2.9.5 Maintenance Process Requirements

**MP-001: Version Control and Release Management**
The system shall maintain semantic versioning (MAJOR.MINOR.PATCH) with documented release notes for each version. Breaking changes shall increment MAJOR version, feature additions shall increment MINOR version, and bug fixes shall increment PATCH version.

**MP-002: Backwards Compatibility Policy**
The system shall maintain data format compatibility across N-1 versions (e.g., version 2.0 must read data written by version 1.9). Breaking data format changes shall be announced 60 days in advance with migration tools provided.

**MP-003: Maintenance Documentation Requirements**
All maintenance activities shall be documented in the project logbook (Section 1.7) including:
- Date and version affected
- Issue type (Adaptive/Corrective/Perfective/Preventive)
- Root cause analysis
- Solution implemented
- Testing performed

**MP-004: Maintenance Resource Allocation**
The project maintenance plan shall allocate development effort using the following distribution:
- 40% Adaptive maintenance (new features, platform updates)
- 30% Corrective maintenance (bug fixes)
- 20% Perfective maintenance (optimization)
- 10% Preventive maintenance (monitoring, testing infrastructure)

Resource allocation shall be reviewed quarterly and adjusted based on actual maintenance workload.

---

===== 2.2.9.6 Maintenance Success Metrics

The effectiveness of maintenance requirements shall be measured using:

| Metric | Target | Measurement Frequency |
|--------|--------|----------------------|
| **Mean Time to Repair (MTTR)** | <48 hours for critical issues | Weekly |
| **Defect Recurrence Rate** | <5% of fixed bugs reappear | Monthly |
| **User-Reported Defect Rate** | <2 defects per 1000 active users | Monthly |
| **Maintenance Cost Ratio** | <30% of total development effort | Quarterly |
| **Platform Compatibility Rate** | 100% on supported OS versions | Per release |
| **Security Vulnerability Exposure** | 0 HIGH/CRITICAL unfixed for >7 days | Continuous |

---

===== 2.2.9.7 Integration with Existing Requirements

**Relationship to Machine Requirements (Section 2.2.5)**:
- Maintenance requirements extend performance requirements by defining how performance targets are maintained over time
- Dependability attributes (when added) will require maintenance procedures for ensuring continued reliability

**Relationship to Interface Requirements (Section 2.2.4)**:
- Adaptive maintenance may require interface requirement updates when new input methods are added
- Corrective maintenance addresses interface defects reported by users

**Relationship to Domain Requirements (Section 2.2.3)**:
- Perfective maintenance improves alignment between system implementation and domain model
- Adaptive maintenance extends domain coverage (new language pairs, new pronunciation features)

**Relationship to Validation (Section 3.2)**:
- Maintenance activities trigger re-validation of affected requirements
- Preventive maintenance includes continuous validation testing
...
=== 2.3.5 User Interface Accessibility and Inclusivity Guidelines

The user interface of the Pronunciation Coach application has been designed with accessibility, inclusivity, and clarity as central principles. The overall layout maintains a clean and organized appearance, ensuring that users can easily identify available features without feeling overwhelmed. Each visual element follows a consistent and intuitive structure, guiding the user naturally through the learning experience.

The interface uses rounded boxes and soft edges to create a friendly and approachable visual environment. A combination of fun colors and simple illustrations supports intuitive navigation and helps users quickly associate icons and actions with specific features. Readable, sans-serif fonts are used throughout the system to improve text legibility and reduce visual strain, accommodating users with reading or vision difficulties.

Accessibility considerations extend to both visual and auditory design elements. The app integrates visual indicators and auditory feedback during quizzes and pronunciation exercises, providing a multisensory learning experience. These features enhance inclusivity by allowing users to learn through the sensory channel that best fits their preferences or abilities. The consistent use of color contrast and clear spacing between interface elements further ensures that users with visual impairments can interact with the system effectively.

By maintaining a straightforward and accessible interface, the Pronunciation Coach supports key software quality attributes such as usability, user satisfaction, and learnability. Its design encourages engagement and inclusivity, ensuring that users of all backgrounds and abilities can benefit from the pronunciation training experience in a comfortable and enjoyable way.
...
==== 3.3.2 Audio Processing Module - Fault Tree Analysis

**Purpose**: This subsection provides detailed technical risk analysis of the pronunciation feedback system using fault tree analysis methodology to identify critical failure modes and their interdependencies.

**Approach**: Fault tree analysis examines the audio processing pipeline by modeling failure propagation from leaf nodes (individual component failures) through intermediate events to the root failure (inaccurate pronunciation feedback). This approach reveals minimal cut sets—combinations of failures that guarantee system malfunction.
...
**Critical Finding:** The system requires ALL three components to function correctly:
1. Clean audio capture
2. Accurate MFCC analysis  
3. Valid native pronunciation models

Failure in ANY component causes system failure (AND logic gate), making this a series reliability configuration.

---

**Identified Leaf Nodes (11 Technical Failures)**

**Audio Capture Issues:**
- Background noise interference (bus, cafeteria environments) - Likelihood: HIGH
- Poor microphone hardware quality - Likelihood: MEDIUM
- Incorrect audio format/codec processing - Likelihood: LOW
- Microphone permission denied by user - Likelihood: MEDIUM

**MFCC Analysis Issues:**
- Incorrect algorithm parameters (window size, hop length) - Likelihood: MEDIUM
- Cross-platform implementation inconsistency (iOS vs Android) - Likelihood: MEDIUM
- flutter_sound library defects or API changes - Likelihood: LOW
- Insufficient device computational resources - Likelihood: LOW

**Native Pronunciation Model Issues:**
- Native audio model missing for target phrase - Likelihood: LOW
- Low quality native speaker recording - Likelihood: MEDIUM
- Pronunciation model language/dialect mismatch - Likelihood: LOW

---

**Cut Set Analysis Results**

Four critical failure combinations identified that will cause inaccurate feedback:

1. **Cut Set 1**: Background noise + Wrong MFCC parameters + Missing native model
2. **Cut Set 2**: Poor microphone quality + Cross-platform inconsistency + Low quality native recording
3. **Cut Set 3**: Background noise + MFCC parameter issues + Model language mismatch
4. **Cut Set 4**: Microphone permission denied + Insufficient resources + Native model quality issues

**Priority Findings:**
- Background noise detection affects 3 of 4 cut sets (75% coverage)
- MFCC parameter validation affects 3 of 4 cut sets (75% coverage)
- Native model quality assurance affects 2 of 4 cut sets (50% coverage)

---

**Mitigation Strategy and Implementation Priority**

Based on fault tree analysis, the following countermeasures are prioritized:

**Priority 1 - High Impact Mitigations:**

1. **Noise Detection System** (addresses Cut Sets 1, 2, 3)
   - Implement pre-recording audio quality check
   - Display "Quiet environment recommended" warning when ambient noise exceeds threshold
   - Provide visual indicator of recording quality during capture
   - *Relation to R1 in Section 3.3.1*: Directly improves speech recognition accuracy

2. **MFCC Parameter Standardization** (addresses Cut Sets 1, 2, 4)
   - Document and validate MFCC configuration across iOS and Android platforms
   - Create unit tests verifying consistent feature extraction results
   - Implement automated parameter validation before pronunciation comparison
   - *Relation to R5 in Section 3.3.1*: Improves processing consistency on slower devices

**Priority 2 - Medium Impact Mitigations:**

3. **Native Model Validation Pipeline** (addresses Cut Sets 1, 2, 4)
   - Establish quality criteria for native speaker recordings (SNR > 20dB, no clipping)
   - Implement automated quality checks during content ingestion
   - Create metadata validation ensuring language/dialect consistency
   - *Relation to R1 in Section 3.3.1*: Ensures reliable reference pronunciation models

4. **Permission and Resource Management** (addresses Cut Set 4)
   - Implement graceful degradation when microphone permission denied
   - Provide clear explanation and recovery path for permission issues
   - Add device capability detection before attempting audio processing
   - *Relation to R2 in Section 3.3.1*: Prevents data loss from incomplete recordings

---

**Monitoring and Validation Plan**

- Conduct **device testing matrix** covering noise scenarios and hardware variations
- Implement **telemetry logging** for MFCC parameter performance across platforms
- Establish **native model quality metrics** reviewed during content updates
- Create **automated fault injection tests** simulating identified leaf node failures

---

**Integration with General Risk Framework**

This fault tree analysis complements the general risk analysis in Section 3.3.1 by providing:

- Technical depth for R1 (Speech Recognition Accuracy) - now decomposed into 11 specific failure modes
- Implementation guidance for R5 (Performance Issues) - addressed through MFCC optimization
- Reliability targets for R2 (Data Loss Prevention) - supported by resource management

The identified cut sets inform the mitigation strategies defined in Section 3.3.1, providing engineering-level detail for technical risk management.
