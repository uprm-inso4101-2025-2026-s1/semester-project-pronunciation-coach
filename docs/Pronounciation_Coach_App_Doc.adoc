...
:revdate: Update: October 23, 2025 
:toc:
:toclevels: 3
:title-page:
link:example$pdf-converter-change-bars.rb[role=include]
...
[.changed]
*Scope*:: Development of a comprehensive Flutter-based mobile application for English pronunciation coaching targeting Spanish-speaking learners, encompassing domain analysis, requirements engineering, software architecture, implementation, testing, and deployment.

[.changed]
*Span*:: The project delivers pronunciation practice features including interactive exercises with real-time feedback using MFCC analysis, gamified learning systems with streak tracking and XP rewards, phoneme-level evaluation with Montreal Forced Alignment, adaptive difficulty algorithms, offline-capable quiz workflows, and progress analytics with CSV export. Technical implementation utilizes Flutter/Dart with Provider state management, SharedPreferences for data persistence, speech-to-text integration (Whisper, Vosk), and enterprise design patterns for maintainable architecture.
...
- Sprint 4: Advanced features (voice cloning, adaptive difficulty)
...
- Performance validation on target devices (Samsung A32 equivalent)
...
*Sprint Planning*: Collaborative feature selection based on completed dependencies, user value, and technical risk assessment
...
[.changed]
*Carlos (45-year-old teacher from Medellín)*: "My high school students giggle when I say 'beach' because it comes out as 'bitch.' I practice every morning at 7 AM before class, using apps that only give me red or green feedback. I need to see tongue placement diagrams - where exactly should my tongue be for the 'ee' sound? The apps I've tried take forever to load pronunciation examples and show generic error messages like 'try again' without telling me what to try differently."

[.changed]
*Technical observation*: During usability testing on a Samsung A32, authentication consistently took 2.8 seconds with basic loading indicators. Users tapped the screen multiple times during delays, assuming the app had crashed. Speech-to-text processing with Whisper took 4.2 seconds for a 3-second audio clip, during which users received no educational content.
...
[.changed]
*Complex Behaviors* (sequences of events and actions):
...
- **Background**: 45-year-old educator preparing to move abroad, embarrassed when students laugh at his pronunciation of "beach" as "bitch"
...
[.changed]
*DR1*: The system must provide means for associating each pronunciation attempt with specific accuracy measurements and articulation guidance derived from comparison to native speaker models

*Domain Property*: Effective pronunciation learning requires quantifiable feedback on specific phonetic errors rather than binary correctness judgments

*DR2*: The system must provide means for tracking consecutive practice days and maintaining visible streak records with adaptive encouragement

*Domain Property*: Consistent daily engagement is essential for pronunciation improvement and requires motivational reinforcement

*DR3*: The system must provide means for displaying educational content during all processing operations exceeding 1 second

*Domain Property*: Learning momentum is maintained when waiting periods are transformed into educational opportunities

*DR4*: The system must provide means for phoneme-level analysis using accent-aware evaluation algorithms

*Domain Property*: Pronunciation errors occur at the phoneme level and vary by native language background

*DR5*: The system must provide means for offline practice capability with synchronization when connectivity resumes

*Domain Property*: Consistent practice requires accessibility in various environments, including those with limited internet connectivity


==== 2.2.4 Interface Requirements

[.changed]
*IR1*: The system must provide means for learners to record pronunciation attempts through device microphone input with visual recording indicators

*Observed Phenomenon*: Maria records herself saying "thought, through, theater" multiple times to compare attempts

*IR2*: The system must provide means for displaying educational content and pronunciation tips during authentication and processing delays

*Observed Phenomenon*: Users currently stare at generic loading spinners for 2-3 seconds during system operations

*IR3*: The system must provide means for showing visual comparisons between learner and native pronunciation waveforms with error highlighting

*Observed Phenomenon*: Carlos needs to see exactly how his "beach" pronunciation differs from native speakers'

*IR4*: The system must provide means for progress visualization through dashboards showing accuracy trends and streak maintenance

*Observed Phenomenon*: Learners check progress indicators to maintain motivation and track improvement patterns

*IR5*: The system must provide means for practice session customization with duration options and word selection
...
User speaks a phrase into the microphone,Audio input captured as a digital recording,Attempt, Recording,onRecordStart(), onRecordEnd(),Stores audio file and initializes analysis pipeline
...
- Carlos's embarrassment with "beach" sounding like "bitch" → **Visual Comparison** concept showing waveform differences and tongue placement
